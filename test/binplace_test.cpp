...
TEST(BinplaceTest, TestBinplaceBasic){
  ...
  std::vector<libUtil::Item<int>> itemized;
  itemized.reserve(n);
  
  for(auto &itr : vec){
    itemized.push_back(libUtil::Item<int>(itr, size_t(itr) % beta, libUtil::ItemType::NORMAL));
  }

  libUtil::Item<int>::BinAssign(itemized, beta, Z);
